# GWRCheck identifies GenericWriteRights and converts the SIDs for easy review, it requires PowerView.ps1 to be imported prior to use.
# Default usage identifies GenericWriteRights for the user executing the script, use -IdentityName= for other users and objects or -h for help.
...
function Show-Usage {
    Write-Host "Usage: .\GWRCheck.ps1 [-IdentityName=<UsernameOrGroup>] [-h]" -ForegroundColor Yellow
    Write-Host "Options:" -ForegroundColor Cyan
    Write-Host "  -IdentityName=<UsernameOrGroup>  Specify the username or group to check for GenericWrite rights. Default is the current user."
...
# Enumerate ACLs and filter for GenericWrite rights for the specified or current user
Get-DomainObjectAcl | Where-Object { 
    $_.ActiveDirectoryRights -eq "GenericWrite" -and $_.SecurityIdentifier -eq $targetSid
} | ForEach-Object {
    [PSCustomObject]@{
        ObjectDN              = $_.ObjectDN
